...
description: This endpoint creates a new signing request. Supports EVM transactions, EIP-712 typed data signing, and EIP-191 message signing.
---
...
| Type | Description |
|------|-------------|
| `evm_tx` | EVM transaction signing (e.g., ERC20 approvals, ERC20 transfers, raw transactions) |
...
| `kind` | string | Conditional | Transaction kind (required for `evm_tx`: `erc20_approve`, `erc20_transfer`, or `raw`) |
...
| `amount` | string | Approval amount (in base units) |
...
| `amount` | string | Transfer amount in base units (e.g., `1000000` for 1.0 USDC) |
| `txOverrides` | object | (Optional) Gas overrides (e.g., `gasLimit`) |
